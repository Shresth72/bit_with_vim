...
func DecodeBencode(bencodedString string, start int) (interface{}, int, error) {
	...
	case bencodedString[i] == 'd':
		return DecodeDict(bencodedString, i)
...
func DecodeDict(b string, st int) (interface{}, int, error) {
	decodedDict := make(map[string]interface{})
	i := st + 1

	for i < len(b) && b[i] != 'e' {
		decodedKey, newSt, err := DecodeString(b, i)
		if err != nil {
			return nil, st, err
		}
		...
		if keyStr, ok := decodedKey.(string); ok {
			decodedDict[keyStr] = decodedValue
		} else {
			return nil, st, fmt.Errorf("bencoded dicrionary key needs to be string")
		}
	}

	if i == len(b) {
		return nil, st, fmt.Errorf("invalid bencoded dict")
	}

	return decodedDict, i + 1, nil
}
