use crate::my_serde_bencode::error::{Error, Result};
use serde::{
...
impl<'de> Deserializer<'de> {
        ...
        T: Neg<Output = T> + AddAssign<T> + MulAssign<T> + From<i8>,
    {
        match self.next_byte()? {
            b'i' => {}
            _ => return Err(Error::Message("expected i".to_string())),
        };
        ...
        let mut int = match self.next_byte()? {
            ...
            _ => return Err(Error::Message("invalid int".to_string())),
        };
...
impl<'de, 'a> de::Deserializer<'de> for &'a mut Deserializer<'de> {
        ...
        V: de::Visitor<'de>,
    {
        if self.next_byte()? == b'l' {
            // give visitor access to each element of the sequence
            let value = visitor.visit_seq(Seq::new(self))?;

            // parse the closing bracked of the sequence
